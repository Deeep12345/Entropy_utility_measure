...
def make_mondrian_config(name, k, vals):
    ...
    <input filename='../../datasets/heart/heart_cat.csv' separator=','/>
    <output filename='../anon_data/heart_randoms/mondrian{name}.csv' format ='genVals'/>
    <id/>
    <qid>"""
    ...
    <sens><att index='15' name='class'/></sens>
    </config>"""
    xml_tree = bs(full_xml, 'xml')
    f = open(f"heart_randoms/mondrian{name}.xml", "w+")
...
def make_datafly_config(name, k, vals, cols, non_ordered_cols, shuffled=False):
    ...
    <input filename='../../datasets/heart/heart_cat.csv' separator=','/>
    <output filename='../anon_data/heart_randoms/datafly{name}{is_shuffled}.csv' format ='genVals'/>
    <id/>
    <qid>"""
    ...
    <sens>
    <att index='15' name='class'/>
    </sens>
    </config>"""

    xml_tree = bs(full_xml, 'xml')
    f = open(f"heart_randoms/datafly{name}{is_shuffled}.xml", "w+")
    f.write(xml_tree.prettify())
    f.close()
...
cols = ['sex', 'age', 'education', 'currentSmoker', 'cigsPerDay', 'BPMeds',
