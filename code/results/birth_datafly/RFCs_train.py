...
def tune(k, X_test_orig, y_test_orig):
    k_data = pd.read_csv(f"../../anon_data/birth_datafly/k{k}_minmaxed.csv").drop("Unnamed: 0", axis=1)
    y = k_data["class"]
...
data = pd.read_csv(f"../../anon_data/birth_datafly/k1_minmaxed.csv").drop("Unnamed: 0", axis=1)
